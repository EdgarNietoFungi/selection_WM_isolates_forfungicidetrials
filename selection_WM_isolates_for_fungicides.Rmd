...
drybeans_tested <- wm1_%>% select(Collection_ID, State, County, Country, Origin, Year, Fungicide_status, Host, Group_MOA_current_season, Group_MOA_previous_seasons) %>% filter(Fungicide_status == "tested", Host== "drybeans") %>% group_by(Host, State, Origin,Year, Group_MOA_current_season,  Group_MOA_previous_seasons) %>% summarize(n=n()) %>%kable(digits = 2)
#q <- drybeans_tested %>%  group_by(State) %>% summarize(n=n())

 
soybeans_tested <- wm1_ %>% select(Collection_ID, State, County, Country, Origin, Year, Fungicide_status, Host, Group_MOA_current_season, Group_MOA_previous_seasons) %>% filter(Fungicide_status == "tested", Host== "soybeans") %>%  group_by(State, Origin, Group_MOA_current_season, Group_MOA_previous_seasons) %>% summarize(n=n())
