...
WM<- WM[-1054, ]# Taking out this column that is in blank
...
#taking 3 samples for each field selected and no taking those isolates already done
selection_nice <- wm2_  %>%
  ungroup() %>%
  filter(
  Field == "po" |
...
dat_long_lat <- selection_chingon %>% 
  select( State, County, Field, long, lat) %>% 
  distinct() 
dat_long_lat <-  dat_long_lat[-c(2,6,12,13,65,67:70,72:78,80:87, 89:97,100:108,110),]
...
#write.csv(df_long_lat, file = "dist_matrix.csv")
#df_long_lat 
```
