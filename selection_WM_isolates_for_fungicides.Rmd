...
wm1_ <-  wm1 %>%
  ...
  group_by(Field) %>% mutate (num_isolates_per_field = ifelse(n() > 2, "2>isolates/Field", "<3isolates/Field")) %>%#Creating a new column if the number of isolates/field is major than 2
  ungroup()
...
selection_FINAL_objective1  + tested (170)
164+ 170
...
selection_FINAL_objective1 <-
  selection3 %>% bind_rows(selection_Mexico3) %>% filter(Fungicide_status == "no_tested")#and also by no fungicide pre-tested
```
...
y = lat,
color = num_isolates_per_field# by color the level of selection
#shape = Yearby shape the level of Year
),
inherit.aes = FALSE,
na.rm = TRUE,
size =1.5
) + theme(legend.position = "bottom") + #facet_wrap(~ Fungicide_status) + #Wrapping by the # of isolates/field
scale_color_manual(values = c("red", "blue")) + theme(legend.position = "bottom") + labs(title = "Greenbean inventory of Sclerotinia sclerotiorum from Sinaloa, Mexico") + theme(plot.title = element_text(
size = 10,
face = "bold",
hjust = 0.5,
family = "Helvetica"
...
#SECOND PART OF DISSERTATION, Evaluate the effect of successive fungicide applications on sensitivity of populations of S. sclerotiorum in field plots
...
##Objective 2 
  Select isolates by the time,  by filterring the points from IA & MI before visualized, make a Venn diagram with allalready used for fungicide trials from objective 1.
 
```{r}
IA_fungicide <- WMfungicide %>% filter(State=="IA",  County =="Floyd"|County =="Chickasaw"|County =="Floyd"|County =="Bremer")
...
#THIRD PART OF DISSERTATION, Characterize the population structure and genetic diversity of S. sclerotiorum populations from soybean throughout the North Central United States and Mexico
  
 . Select hierarchieally isolates by space according to genotype 
 
```{r, warning= FALSE}
# Combining both
selection_FINAL_objective3_genotyping <-
