...
## THE FIRST PART OF DISSERTATION, Survey fungicide sensitivity of populations of S. sclerotiorum from soybean
#Background
...
wm1_ <-  wm1 %>%
  ...
  group_by(Field) %>% mutate (num_isolates_per_field = ifelse(n() > 2, "3 ≥isolates/Field", "<3isolates/Field")) %>%#Creating a new column if the number of isolates/field is major than 2
  ungroup()
...
the __hypothesis of this objective is that fungicide applications increase genetic diversity of _S. sclerotiorum_ population by field-applied fungicide doses on stress-induced mutation 
