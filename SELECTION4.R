...
levels(WM$DNA.Extraction )[levels(WM$DNA.Extraction ) == ""] <- "No"
...
wm1_ <-  wm1 %>%
  mutate(Status = ifelse(Collection_ID  %in% tested,
                         "tested", "no_tested")) %>%
  filter(!Collection_ID %in% mexican_isolates) %>%
  group_by(Field) %>% mutate (num_isolates_per_field = ifelse(n() > 9, ">than_9iso", 
                                                              ifelse(n() > 2, ">than_2,<than_9iso", "<than_2iso"))) %>% ungroup()
...
wm1_$Status <- as.factor(wm1_$Status)
...
mid_west_base + geom_point(data = wm1_, aes(x = long, y = lat),inherit.aes = FALSE, color = "red", size = 0.3, na.rm = TRUE, shape=22, stroke= 0.5, fill= "white")  + labs(title = "Sclerotinia sclerotiorum Soybean inventory in Midwest") + theme(plot.title = element_text(
size = 10,
face = "bold",
hjust = 0.5,
family = "Verdana"
...
#by origin
mid_west_base + geom_point(data = wm1_, aes(x = long, y = lat, color= Origin),inherit.aes = FALSE, size = 1, na.rm = TRUE, shape=22, stroke= 0.5, fill= "white") +  theme(legend.position = "bottom") + labs(title = "Sclerotinia sclerotiorum Soybean inventory in Midwest") + theme(plot.title = element_text(
  size = 10,
  face = "bold",
  hjust = 0.5,
  family = "Verdana"
...
#by year
mid_west_base + geom_point(data = wm1_, aes(x = long, y = lat, color= Year),inherit.aes = FALSE, size = 1,  na.rm = TRUE, shape=22, stroke= 0.5, fill= "white")+ theme(legend.position = "bottom") + labs(title = "Sclerotinia sclerotiorum Soybean inventory in Midwest") + theme(plot.title = element_text(
  size = 10,
  face = "bold",
  hjust = 0.5,
  family = "Verdana"
...
mid_west_base + geom_point(data = wm1_, aes(x = long, y = lat, color= Origin),inherit.aes = FALSE, size = 1, na.rm = TRUE, shape=22, stroke= 0.5, fill= "white")+ facet_wrap(~Year) + theme(legend.position = "bottom") + labs(title = "Sclerotinia sclerotiorum Soybean inventory in Midwest") + theme(plot.title = element_text(
  size = 10,
  face = "bold",
  hjust = 0.5,
  family = "Verdana"
...
mid_west_base + geom_point(data = WMfarmerfields, aes(x = long, y = lat, color= Year),inherit.aes = FALSE, size = 1, na.rm = TRUE, shape=22, stroke= 0.5, fill= "white") + theme(legend.position = "bottom")


#fungiicde field trials
WMfungicide <- wm1_ %>% 
  group_by(State,Year, Origin) %>%   
...
mid_west_base + geom_point(data = WMfungicide, aes(x = long, y = lat, color= Year),inherit.aes = FALSE, size = 1, na.rm = TRUE, shape=22, stroke= 0.5, fill= "white") + theme(legend.position = "bottom")
...
NE_counties <- counties %>% group_by(region) %>% filter( region == "nebraska") %>% ungroup %>% filter(subregion=="antelope"|subregion=="madison"|subregion=="cedar"|subregion=="cuming"|subregion=="dodge"|subregion=="boone")
NE_counties_base <-  ggplot(data = NE_counties,
                            mapping = aes(x = long, y = lat, group = group)) +
  coord_fixed(1.3) +
  geom_polygon(color = "black", fill = "grey")

NE_counties_base + geom_point(
  data = WM1S,
  aes(
    x = long,
    y = lat,
...
IA_county_base + geom_point(
  ...
  scale_color_manual(values = c("red", "blue", "green")) + facet_wrap(~ Status)  
### By DNA extraction
IA_county_base + geom_point(
  data = WM_IA,
  aes(
...
IA_county_base + geom_point(
  ...
  size = 1.5) +
  theme(legend.position = "bottom")+ 
  scale_color_manual(values = c("red", "blue", "green")) + facet_wrap(~ DNA.Extraction)

#MAKE A MAP FOR THE PRINCIPAL COUNTIES FOR IOWA
IA_counties <- counties %>% group_by(region) %>% filter( region == "iowa") %>% ungroup %>% filter(subregion=="floyd"|subregion=="butler"|subregion=="chickasaw"|subregion=="bremer")
IA_counties_base <-  ggplot(data = IA_counties,
                            mapping = aes(x = long, y = lat, group = group)) +
  coord_fixed(1.3) +
  geom_polygon(color = "black", fill = "grey")

IA_counties_base + geom_point(
  data = WM_IA,
  aes(
    x = long,
    y = lat,
...
WM_county_base <- ggplot(data = WM_WIcounty, mapping = aes(x = long, y = lat, group = group)) +
  coord_fixed(1.3) +
  geom_polygon(color = "black", fill = "white")
...
WM_county_base + geom_point(
  ...
  scale_color_manual(values = c("red", "blue", "green")) + facet_wrap(~ Status) #Status  
...
WI_counties <- counties %>% group_by(region) %>% filter( region == "wisconsin") %>% ungroup %>% filter(subregion=="walworth"|subregion=="grant"|subregion=="lafayette")
WI_counties_base <-  ggplot(data = WI_counties,
                            mapping = aes(x = long, y = lat, group = group)) +
  coord_fixed(1.3) +
  geom_polygon(color = "black", fill = "grey")

WI_counties_base + geom_point(
  data = WM_WI,
  aes(
    x = long,
    y = lat,
...
MI_counties <- counties %>% group_by(region) %>% filter( region == "michigan") %>% ungroup %>% filter(subregion=="montcalm"|subregion=="ingham")
MI_counties_base <-  ggplot(data = MI_counties,
                            mapping = aes(x = long, y = lat, group = group)) +
  coord_fixed(1.3) +
  geom_polygon(color = "black", fill = "grey")

MI_counties_base + geom_point(
  data = WM_MI,
  aes(
    x = long,
    y = lat,
