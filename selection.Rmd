...
```{r}
###
WM <- read.csv("data/WM-fixingproductnamesID2573_modifyingforactiveingredient_7.csv", stringsAsFactors = TRUE)# Making the columns as 
...
State== "MI"| State== "ND"| State== "WA"| State== "NE"| State== "CO"| State== "OR"| State== "CA"| State== "MN"| State== "WI"| State== "ID"| State== "NY"| Country== "mexico"| Country == "brazil") 
...
```{r}
 drybeans_no_tested %>% filter(Field=="WM_production_1"|
                              Field=="WM_production_2"|
                                ...
                                Field=="WM_production_27"|
                                Field=="WM_production_28") %>%
  select(Host,Country, State, Year, Collection_ID, Field ) %>% 
  group_by( Collection_ID, Field) %>%  summarize() 
#%>% 
