...
wm1 <-
  WM %>% select(c(Collection_ID, State, Field, Year, Origin, long, lat, hyphal_tip..ht., DNA.Extraction, County, Serial_agar_dilution_for_creating_the_model_..N.20., Form.ID))
...
# #zzz <-
#   wm1_  %>% group_by(State, County, Field) %>%  summarize (num_ISO_per_field = n()) %>% add_count (County) %>%  ungroup %>% rename (num_fields_per_county = n) %>%  arrange(factor (State, levels = c("NE", "IA", "WI", "MI")))
# 
# WM2 <- WM %>% select(-Apparently.low.sensitiviy)

ddd <-
    wm1_ %>% 
   group_by(State, County, Field) %>%
    summarize(num_ISO_per_field = n()) %>%
      add_count(County) %>%
        ungroup %>%
           rename (num_fields_per_county = n) %>%
             right_join (wm1_, by = "Field") %>%
                arrange(factor (State.x, levels = c("NE", "IA", "WI", "MI"))) %>%
                    select(Collection_ID,State.x, County.x, Field, num_ISO_per_field, num_fields_per_county, Year,  Origin, long, lat, hyphal_tip..ht., DNA.Extraction, Serial_agar_dilution_for_creating_the_model_..N.20., Fungicide_status,Form.ID) %>% 
                       rename( State = State.x, County = County.x) %>%
                 # group_by(State, County, Field) %>% 
                          # arrange( desc(County))
dddfilter1 <- ddd %>%
    filter(!num_ISO_per_field <= 2) %>% 
...
M2S <-  WM1S  %>% group_by(State, County, Field) %>%  summarize( num_ISO_per_field = n()) %>% add_count(County) %>%  ungroup %>% rename ( num_fields_per_county= n) 

WM2S$num_ISO_per_field <-   as.numeric( WM2S$num_ISO_per_field)
WM2S <- WM2S %>% filter(num_ISO_per_field>= 2)
...
WM_IA2 <-  WM_IA   %>% group_by(State, County, Field) %>%  summarize( num_ISO_per_field = n()) %>% add_count(County) %>%  ungroup %>% rename ( num_fields_per_county= n) 

WM_IA2$num_ISO_per_field <-   as.numeric( WM_IA2$num_ISO_per_field)
WM_IA2 <- WM_IA2 %>% filter(num_ISO_per_field>= 2)
##
...
WM_WI2 <-  WM_WI  %>% group_by(State, County, Field) %>%  summarize( num_ISO_per_field = n()) %>% add_count(County) %>%  ungroup %>% rename ( num_fields_per_county= n) 

WM_WI2$num_ISO_per_field <-   as.numeric( WM_WI2$num_ISO_per_field)
WM_WI2 <- WM_WI2 %>% filter(num_ISO_per_field>= 2)
...
WM_MI2 <-  WM_MI  %>% group_by(State, County, Field) %>%  summarize( num_ISO_per_field = n()) %>% add_count(County) %>%  ungroup %>% rename ( num_fields_per_county= n) 

WM_MI2$num_ISO_per_field <-   as.numeric( WM_MI2$num_ISO_per_field)
WM_MI2 <- WM_MI2 %>% filter(num_ISO_per_field>= 2)
...
ddd <-
    wm1_ %>% group_by(State, County, Field) %>%  summarize(num_ISO_per_field = n()) %>% add_count(County) %>%  ungroup %>% rename (num_fields_per_county = n) %>% right_join (WM2, by = "Field") %>% arrange(factor (State.x, levels = c("NE", "IA", "WI", "MI"))) %>% select(Collection_ID,State.x,  Field, County.x, Year, Origin, long, lat, num_ISO_per_field, num_fields_per_county, hyphal_tip..ht., DNA.Extraction,  Form.ID, Serial_agar_dilution_for_creating_the_model_..N.20.) %>% rename( State = State.x)
